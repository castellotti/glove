...
def _render_vibe(
    ...
    doc: dict[str, Any] = {
        ...
        # hook, which rewrites the command text, applies cleanly (PLAN §5.3).
    ...
    # per-command sandbox (PLAN §5.3). Only when an in-container enforcer is
...
# Routes every shell command through the ring-1 enforcer (PLAN §5.3). The hook
