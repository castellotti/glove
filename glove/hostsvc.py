...
def _expand(command: str, cfg: Config, session: str) -> str:
    """Expand {placeholders} so presets stay session-agnostic."""
    for key, val in _tokens(cfg, session).items():
        command = command.replace("{" + key + "}", val)
    return command


def _tokens(cfg: Config, session: str) -> dict[str, str]:
    workdir = os.path.realpath(cfg.workdir)
    research = os.path.join(workdir, "research", cfg.resolved_collection())
    return {
        "session": session,
        "workdir": workdir,
        "collection": cfg.resolved_collection(),
        "research_dir": research,
        "media_dir": os.path.join(research, "media"),
        "filtered_dir": os.path.join(research, "filtered"),
        "chrome_profile": str(Path.home() / ".glove" / "chrome-profile"),
        "home": str(Path.home()),
    }


def media_dir_for(cfg: Config, session: str) -> Path:
    return Path(_tokens(cfg, session)["media_dir"])
...
def start_host_services(cfg: Config, session: str, session_dir: Path) -> None:
    ...
    # Ensure the screenshot output dir exists so Playwright's --output-dir is valid.
    media_dir_for(cfg, session).mkdir(parents=True, exist_ok=True)
    ...
    for svc in cfg.host_services:
        ...
        command = _expand(svc.command, cfg, session)
        if have_tmux:
...
def describe_host_services(cfg: Config, session: str) -> None:
    ...
    for svc in cfg.host_services:
        ...
        console.print(f"    {_expand(svc.command, cfg, session)}")
